...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, TextInput, Image, Dimensions, ActivityIndicator, Alert } from 'react-native';
...
interface Professional { id: string; name: string; rating: number | null; reviewCount: number; distance: string; price: string; specialties: string[]; avatar: string; verified: boolean; responseTime: string; }
...
export default function HomeScreen() {
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      if (status !== 'granted') { setLocation('Permissão negada'); return; }
  ...
  const renderProfessionalCard = (professional: Professional) => (
    <TouchableOpacity key={professional.id} style={styles.professionalCard} onPress={() => { /* Navegação futura */ }}>
      <View style={styles.professionalHeader}>
        <Image source={{ uri: professional.avatar }} style={styles.avatar} />
        <View style={styles.professionalInfo}>
          <View style={styles.nameContainer}>
            <Text style={styles.professionalName}>{professional.name}</Text>
            {professional.verified && (<View style={styles.verifiedBadge}><Text style={styles.verifiedText}>✓</Text></View>)}
          </View>
          <View style={styles.ratingContainer}>
            <Star size={14} color={professional.rating ? "#FFA500" : "#CBD5E1"} fill={professional.rating ? "#FFA500" : "transparent"} />
            
            {/* A CORREÇÃO ESTÁ AQUI */}
            <Text style={styles.rating}>
              {professional.rating ? professional.rating.toFixed(1) : 'Novo'}
            </Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}><View style={styles.locationContainer}><MapPin size={20} color="#2563EB" /><Text style={styles.locationText}>{location}</Text></View><TouchableOpacity style={styles.filterButton}><Filter size={20} color="#64748B" /></TouchableOpacity></View>
      <View style={styles.searchContainer}><View style={styles.searchInputContainer}><Search size={20} color="#64748B" /><TextInput style={styles.searchInput} placeholder="Buscar profissionais ou serviços..." value={searchQuery} onChangeText={setSearchQuery} /></View></View>
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.quickActions}><TouchableOpacity style={styles.quickActionButton} onPress={() => router.push('/service-request')}><View style={styles.quickActionIcon}><Plus size={24} color="#2563EB" /></View><Text style={styles.quickActionText}>Solicitar Novo Serviço</Text></TouchableOpacity></View>
        <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>Profissionais Próximos</Text>{!loading && <Text style={styles.sectionSubtitle}>{professionals.length} profissionais disponíveis</Text>}</View>
        {loading ? <ActivityIndicator size="large" color="#2563EB" style={{ marginVertical: 20 }}/> : <View style={styles.professionalsContainer}>{professionals.map(renderProfessionalCard)}</View>}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  contactButton: { backgroundColor: '#2563EB', borderRadius: 8, paddingHorizontal: 16